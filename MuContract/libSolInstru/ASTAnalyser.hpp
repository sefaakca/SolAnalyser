...
class ASTAnalyser {
public:
    ASTAnalyser(std::stringstream& _ast_sstream, nlohmann::json& _jsonast, std::string _mutation_type, int _target_function_id, bool single_file = false, std::string file_name = "");
...
private:
    ...
    ParameterListNodePtr handle_parameter_list_for_events();
    ...
    std::string get_extra_code_for_mutation(std::string version);
};
